    ...
    <div class="l-content-wrapper">
        <div class="l-container">
          <div class="l-content">
            ...
            <form method="POST" action="{{ route('steps.update', $step->id) }}" class="c-form">
                @csrf
                ...
                <fieldset class="c-form__field">
                  ...
                  @enderror
                  <textarea name="description" id="description" class="c-form__field__textarea @error('description') is-invalid @enderror" style="min-height: 8rem;" placeholder="学びのステップに関するサマリー。対象者や難易度、得られるものについて。">{{ old('description', isset($step->description) ? $step->description : '') }}</textarea>
                </fieldset>
                  ...
                  <div class="c-selectbox type01">
                    <select name="category_id" id="category" class="@error('category_id') is-invalid @enderror">
                      @foreach ($categories as $category)
                        ...
                        @elseif($step->category_id == $category->id)
                          <option value = "{{ $category->id }}" selected>{{ $category->name }}</option>
                        @else
                ...
                <div class="c-form__field">
                  ...
                  <div class="c-form__child-item">
                    ...
                    <div class="c-form__child-item__right">
                      <input id="estimated_achievement_day" type="text" name="estimated_achievement_day" placeholder="例：3" value="{{ old('estimated_achievement_day', isset($step->estimated_achievement_day) ? $step->estimated_achievement_day : '') }}" class="@error('estimated_achievement_day') is-invalid @enderror">
                  ...
                  <div class="c-form__child-item">
                    ...
                    <div class="c-form__child-item__right">
                      <input id="estimated_achievement_hour" type="text" name="estimated_achievement_hour" placeholder="例：6" value="{{ old('estimated_achievement_hour', isset($step->estimated_achievement_hour) ? $step->estimated_achievement_hour : '') }}" class="@error('estimated_achievement_hour') is-invalid @enderror">
                ...
                @for ($i = 1; $i <= $child_steps_num; $i++)
                  <fieldset class="c-form__field">
                    ...
                    @enderror
                    <input id="child-step{{ $i }}-title" type="text" name="child-step{{ $i }}-title" value="{{ old('child-step'.$i.'-title', isset($child_steps[$i-1]['title']) ? $child_steps[$i-1]['title'] : '') }}" class="@error('child-step'.$i.'-title') is-invalid @enderror" placeholder="学ぶ手順の見出し">
                  </fieldset>
                  ...
                  @enderror
                  <fieldset class="c-form__field">
                    <textarea name="child-step{{ $i }}-description" id="child-step{{ $i }}-description" class="c-form__field__textarea  @error('child-step'.$i.'-description') is-invalid @enderror" style="min-height: 8rem;" placeholder="具体的にやること、コツやポイントについて">{{ old('child-step'.$i.'-description', isset($child_steps[$i-1]['description']) ? $child_steps[$i-1]['description'] : '') }}</textarea>
                  </fieldset>
                @endfor
